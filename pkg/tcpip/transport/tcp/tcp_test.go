...
func TestTCPDeferAccept(t *testing.T) {
	...
	if _, _, err := c.EP.Accept(nil); err != tcpip.ErrWouldBlock {
		t.Fatalf("c.EP.Accept(nil) returned unexpected error got: %s, want: %s", err, tcpip.ErrWouldBlock)
	}
	...
	if err != nil {
		t.Fatalf("c.EP.Accept(nil) returned unexpected error got: %s, want: nil", err)
	}
